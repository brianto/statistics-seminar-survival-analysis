...
Similarly, the cumulative risk estimator $\hat \Lambda{t}$ can be estimated given our sample data. An estimate for the cumulative risk is the cumulative sum of $\frac{n - d}{n}$ which can be easily computed from the table above. This is called the Nelson-Aalen estimator.
...
Variants of the proportional hazards model can be made by varying the baseline hazard's distribution. For example, if $\lambda_{0}$ follows an exponential distribution, then the hazard function $\lambda$ follows a Weibull distribution. Likewise, if $\lambda_{0}$ is constant, then $\lambda$ follows an exponential distribution.
...
We can view the $\mathbf{x' \beta}$ factor in the model above as a modifier for the time-to-event $T$ that streatches and shrinks the survival rate horizontally.
...
A sample of the full data set is shown below.
...
Below is a graph of survival data for all points with no explanatory variables in the model.
...
We may verify this casual relation by performing a log-rank test assuming $\alpha = 0.05$ and where the hypotheses are:

\[ H_0: \Lambda_{ys \leq 12} = \Lambda_{ys > 12} \]
\[ H_a: \Lambda_{ys \leq 12} \neq \Lambda_{ys > 12} \]
...
\[ H_0: \Lambda_{p = 1} = \Lambda_{p = 0} \]
\[ H_a: \Lambda_{p = 1} \neq \Lambda_{p = 0} \]

\scriptsize
<<bfeed-poverty-test, echo = FALSE>>=
