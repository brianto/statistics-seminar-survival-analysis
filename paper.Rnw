...
While many software packages can easily create survival estimates, computing $\hat S(t)$ can be relatively straightforward using tables. Let's examine sample data shown as Figure \ref{fig:ws}.

\begin{figure}[p]
<<ws, echo = FALSE, fig = TRUE, height = 3>>=
