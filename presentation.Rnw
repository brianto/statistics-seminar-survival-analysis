...
bfeed.poverty.short <-
  ...
  ggtitle('Breast Feeding Duration by Poverty, Duration < 30 Weeks') +
  xlab('Duration in Weeks') +
  ylab('Survival') +
  geom_vline(aes(xintercept = 27),
    colour = 'red',
...
bfeed.poverty.all <-
  ggsurv(survfit(Surv(duration, delta) ~ poverty, data = subset(bfeed, duration > 20))) +
...
% semi-parametric
% hazards function given covariates (factors)
% H_0: all \Beta_i = 0

$$\lambda(t | \mathbf{x}) = \lambda_{0}(t) \exp \left( \mathbf{x' \beta} \right)$$

Accelerated Failure Time Model
% parametric, specifies log(T)
%   exponential
%   weibull
%   lognormal
%   log-log
