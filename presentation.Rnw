...
\begin{center}
<<example-timeline, echo=FALSE, fig=TRUE, height=4, width=6>>=
...
Assume $f(t)$ is the instantaneous failure rate

\vspace{0.5in}

% Hazard stuff is wrong
\begin{tabular}{lllcc}
Survival
  & $P(\mbox{no event in } t_i < t)$
  & $S(t)$ & = & $\int^{t}_{0} f(x)$ \\
Hazard
  & $P(\mbox{no event until } t)$
  & $\lambda(t)$ & = & $^{f(t)} / _{S(t)}$ \\
Cum. \\ Hazard
  & E()
...
\begin{frame}{Example}
\begin{center}
<<bfeed-plain, echo=FALSE, fig=TRUE, height=4, width=6>>=
